// DeleteConfirmModal.jsx - Confirmation modal for deleting officer
import React, { useState } from 'react';
import { AlertTriangle, Trash2, X } from 'lucide-react';

const DeleteConfirmModal = ({ isOpen, onClose, officer, onConfirm }) => {
  ...
  const handleConfirm = async () => {
    try {
      setIsDeleting(true);
      setError('');
      await onConfirm(officer.userId || officer.id);
      onClose();
    } catch (err) {
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            <div className="space-y-2">
              <div>
                ...
                <p className="font-medium text-gray-900">{officer.officerId}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium text-gray-900">{officer.mobileNumber}</p>
              </div>
              {officer.email && (
                <div>
              ...
              {officer.selectedCouncil && (
                <div>
                  <p className="text-sm text-gray-500">Municipal Council</p>
                  <p className="font-medium text-gray-900">{officer.selectedCouncil}</p>
                </div>
              )}
            </div>
          </div>
